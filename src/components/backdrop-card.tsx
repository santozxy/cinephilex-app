...
export default function BackdropCard({
  ...
  return (
    <section className="w-full h-[40rem] max-sm:h-64 relative">
      <Image
        priority
        src={originalImageURL + itemHighPopularity.backdrop_path}
        width={1280}
        height={720}
        alt={
          itemHighPopularity.title || itemHighPopularity.name || "Sem título"
        }
        className="object-fill object-center w-full h-full"
      />

      <Link className="z-[99]" href={`/movies/${itemHighPopularity.id}`}>
        ...
        <div className="flex flex-col gap-4 sm:w-96 p-2 rounded-lg">
          <div className="flex sm:flex-col items-start gap-4">
            ...
            <h1 className="sm:text-xl uppercase font-extrabold">
              {itemHighPopularity.title ||
                itemHighPopularity.name ||
                "Sem título"}
            </h1>
          </div>
          <p className="text-zinc-300 text-justify max-sm:hidden">
            {" "}
            {itemHighPopularity.overview.slice(0, 202) + "..."}
          </p>
          <Link
            ...
            href={`/movies/${itemHighPopularity.id}`}
          >
